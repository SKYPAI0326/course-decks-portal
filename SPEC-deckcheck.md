...
| 2026-05-11 | v1.2 | **B+C 完成 ERROR 94→0**：B 步驟 canonical 加 callout typography fork 註記（56% deck 已採用 1.4vh padding）；C 步驟 deckcheck.yml 加 11 條課程級 `allowed_findings` 例外（dm70h / ntub-gtm / gen-ai-140h / gen-ai-36h / gtm / ai-workshop / gemini-ai / line-stickers / ipas 等）+ 新增 `exceptions.py` filter（match → 降為 INFO + 標 `_allowed/_reason/_expires`），所有例外帶 `expires: 2026-08-01`。 |
| 2026-05-11 | v1.3 | **Phase 2 a/b/c 完成**：(a) `visual_regression` checker + Pillow pixel diff，第一次跑建 baseline、之後 compare（pixel diff >0.5% WARN, >2% ERROR）；deterministic 化（停掉 Motion One playSlide、setProperty important 鎖死 opacity/transform）。(b) `mobile_gesture`（393×852 viewport 驗 reader scroll 暢通）+ `raf_drain`（3 秒 RAF >60 → ERROR）；新 profile `mobile`。(c) `.github/workflows/deckcheck.yml` push/PR 自動 fast、workflow_dispatch 可手動 normal/full/visual/mobile，artifact 30 天。 |
| 2026-05-11 | v1.4 | **Phase 2 收尾**：CI fix（DECKCHECK_DECKS_PARENT env override + framework-sanity workflow + workflow_dispatch 自動 clone 16 repo）+ Contrast tuning（忽略 .chrome/.foot/.kicker/.meta-row 等 design-intent metadata + threshold 全壓到 < 2.0 才報，全站 16498→0 contrast 偽陽性）+ 830 PNG visual baseline 建構完成（83 MB，gitignored，deterministic 0 diff）。Final 健檢：1 BLOCKER（已知邊界）/ 0 ERROR / 1398 WARN（多為 static_lint 啟發式偽陽性）。 |
